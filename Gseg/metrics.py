...
def flow_error(maski, dP_net, use_gpu=False, device=None):
    ...
    dP_masks = dynamics.masks_to_flows(maski, use_gpu=use_gpu, device=device)[:2]
    # difference between predicted flows vs mask flows
    flow_errors=np.zeros(maski.max())
    for i in range(dP_masks.shape[0]):
        flow_errors += mean((dP_masks[i] - dP_net[i])**2, maski,
...
def compute_IoU(masks_preds, test_dir):
    _, labels, _, _, _, _, _, _, _, _ = Gseg_io.load_train_test_data(test_dir, 0)
