...
def use_gpu(gpu_number=0, use_torch=True):
    ...
    if use_torch:
        ...
        raise ValueError('Gseg only runs with pytorch now')
...
class UnetModel():
    def __init__(self, gpu=False, pretrained_model=False,
                    diam_mean=34., net_avg=False, device=None,
...
#         slc = [slice(0, self.nclasses) for n in range(imgs.ndim)] # changed from imgs.shape[n]+1 for first slice size 
    ...
    def _train_step(self, x, lbl, iepoch):
        '''
        x: (8,5,224,224)
        lbl:(8,3,224,224)
        '''
        ...
        self.net.train()
        y = self.net(X[:,:4])[0]
        loss = self.loss_fn(lbl, y, x[:,4] ,iepoch)
        loss.backward()
    ...
    def _test_eval(self, x, lbl, iepoch):
        X = self._to_device(x)
        self.net.eval()
        with torch.no_grad():
            y, style = self.net(X[:,:4])
            loss = self.loss_fn(lbl, y, x[:,4], iepoch)
            test_loss = loss.item()
            test_loss *= len(x)
        return test_loss
    ...
    def _train_net(self, train_data, train_labels, 
        ...
        # print("train_data_train_net:", np.array(train_data).shape) #(1316,5,256,256)
        # print("train_labels_train_net:", np.array(train_labels).shape) #(1316,4,256,256)
        # print("test_data_train_net:", np.array(test_data).shape) #(328,5,256,256)
        # print("test_labels_train_net:", np.array(test_labels).shape) #(328,4,256,256)
        ...
        for iepoch in range(self.n_epochs):    
            ...
            for ibatch in range(0,nimg_per_epoch,batch_size):
                ...
                # print("imgi_train_net:", imgi.shape) #(8,5,224,224)
                # print("lbl_train_net:", lbl.shape) #(8,3,224,224)
                train_loss = self._train_step(imgi, lbl, iepoch)
                lavg += train_loss
                nsum += len(imgi) 
            ...
            if iepoch%10==0 or iepoch==5:
                ...
                if test_data is not None:
                    ...
                    for ibatch in range(0,len(test_data),batch_size):
                        ...
                        test_loss = self._test_eval(imgi, lbl, iepoch)
                        lavgt += test_loss
                        nsum += len(imgi)
