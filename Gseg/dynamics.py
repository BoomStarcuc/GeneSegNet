...
def _extend_centers_gpu(neighbors, centers, isneighbor, Ly, Lx, n_iter=200, device=torch.device('cuda')):
    ...
    # print("_extend_centers_gpu_dy_squeeze(0):", dy.squeeze(0).shape)
    # print("_extend_centers_gpu_dx_squeeze(0):", dx.squeeze(0).shape)
    # print("_extend_centers_gpu_dy_squeeze():", dy.squeeze().shape)
    # print("_extend_centers_gpu_dx_squeeze():", dx.squeeze().shape)
    mu_torch = np.stack((dy.cpu().squeeze(0), dx.cpu().squeeze(0)), axis=-2)
    # print("_extend_centers_gpu_dx:", mu_torch.shape)
    return mu_torch
...
def masks_to_flows_gpu(masks, device=None):
    ...
    # run diffusion
    # print("masks_to_flows_gpu_neighbors:", neighbors.shape)
    # print("masks_to_flows_gpu_centers:", centers.shape)
    # print("masks_to_flows_gpu_isneighbor:", isneighbor.shape)
    # print("Ly Lx:",  Ly, Lx)
...
def labels_to_flows(labels, files=None, use_gpu=False, device=None, redo_flows=False):
    ...
    if labels[0].shape[0] == 1 or labels[0].ndim < 3 or redo_flows: # flows need to be recomputed
        ...
        # print("dy_labels_renumber:", fastremap.renumber(labels[0], in_place=True)[0])
        # print("dy_labels_renumber:", fastremap.renumber(labels[0], in_place=True)[0].shape)
        # print("dy_labels_renumber_max:", fastremap.renumber(labels[0], in_place=True)[0].max())
        # print("dy_labels_renumber_min:", fastremap.renumber(labels[0], in_place=True)[0].min())
