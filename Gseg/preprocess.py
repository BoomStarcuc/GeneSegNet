...
def sliding_window_inference(inputs, label, spot, spot_in_cell, roi_size, sw_batch_size, sigma, save_dir, device):
    ...
    for slice_index in range(0, len(slices), sw_batch_size):
        ...
        for curr_index in slice_index_range:
            ...
            if (not os.path.exists("{}/GauMaps/GauMap/{}_gaumap.jpg".format(save_dir,curr_index)) or 
                not os.path.exists("{}/GauMaps/GauMap_all/{}_gaumap_all.jpg".format(save_dir, curr_index)) or
                not os.path.exists("{}/images/{}_image.jpg".format(save_dir, curr_index)) or
                not os.path.exists("{}/labels/{}_label.png".format(save_dir, curr_index)) or
                not os.path.exists("{}/spots/{}_spot.csv".format(save_dir, curr_index))):
                ...
                with open('{}/spots/{}_spot.csv'.format(save_dir, curr_index), 'w', newline='') as csvfile:
                ...
                # print("lb:", lb.shape, lb.dtype)
                # print("crop_spots:", crop_spots.shape, crop_spots.dtype)
                # print("gaumap:", gaumap.shape, gaumap.dtype)
                # print("gaumap_all:", gaumap_all.shape, gaumap_all.dtype)
                gaumap = gaumap*255
                gaumap_all = gaumap_all*255
            else:
                #print("---------------------------existed------------------------------------")
                im = cv2.imread("{}/images/{}_image.jpg".format(save_dir, curr_index))
                im = torch.from_numpy(im).permute((2,0,1)).to(device)

                lb = cv2.imread("{}/labels/{}_label.png".format(save_dir, curr_index), -1)
                lb = torch.from_numpy(lb.astype(np.int32)).to(device)

                with open('{}/spots/{}_spot.csv'.format(save_dir, curr_index), 'r') as f:
                    lines = f.readlines()
                    lines = lines[1:]
                ...
                for line in lines:
                    ...
                    spot_list.append([splits[0], splits[1]])
                        
                crop_spots = np.array(spot_list).astype(np.int32)
                crop_spots = torch.from_numpy(crop_spots).to(device)

                gaumap = cv2.imread("{}/GauMaps/GauMap/{}_gaumap.jpg".format(save_dir, curr_index) ,0)
                gaumap = torch.from_numpy(gaumap).float().to(device)

                gaumap_all = cv2.imread("{}/GauMaps/GauMap_all/{}_gaumap_all.jpg".format(save_dir, curr_index) ,0)
                gaumap_all = torch.from_numpy(gaumap_all).float().to(device)
...
def preprocessing(image, label, spot, sigma, roi_size, sw_batch_size, save_dir, image_dir, return_type = 0, device='cpu'):
    foldername = os.path.splitext(os.path.basename(image_dir))[0]
    ...
    C_images, C_labels, C_spots, C_gaussian, C_gaussian_all = sliding_window_inference(image, label, spot, spot_in_cell, roi_size, 
                                                                                        sw_batch_size, sigma, save_dir, device)
