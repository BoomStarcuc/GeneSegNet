...
def get_label_files(folder, N, mask_filter='_label', foldername = 'output', variance = 7):
    label_names = []
    
    label_path = os.path.join(folder, '{}_{}_Var{}'.format(foldername, N, variance))
    ...
    if not os.path.exists(label_path):
        ...
        print("#################enter output @@@@@@@@@@@@@@@@@@@@@@@@@")
        label_names.extend(glob.glob(os.path.join(folder, '{}_{}_Var{}'.format(foldername, N, variance)) + '/*%s.png'%mask_filter))
        label_names.extend(glob.glob(os.path.join(folder, '{}_{}_Var{}'.format(foldername, N, variance)) + '/*%s.jpg'%mask_filter))
        label_names.extend(glob.glob(os.path.join(folder, '{}_{}_Var{}'.format(foldername, N, variance)) + '/*%s.jpeg'%mask_filter))
        label_names.extend(glob.glob(os.path.join(folder, '{}_{}_Var{}'.format(foldername, N, variance)) + '/*%s.tif'%mask_filter))
        label_names.extend(glob.glob(os.path.join(folder, '{}_{}_Var{}'.format(foldername, N, variance)) + '/*%s.tiff'%mask_filter))
...
def load_images_labels_gaussian(dir, N, mask_filter='_masks', image_filter='_label', gaussian_filter='_gaussian', foldername = 'output', variance = 7):
    image_names = get_image_files(dir, image_filter)
    label_names, flow_names = get_label_files(dir, N, mask_filter, foldername, variance)
...
def load_train_test_data(train_dir, N, test_dir=None, image_filter='_image', mask_filter='_label', gaussian_filter='_gaussian', foldername = 'output', variance = 7):
    #train data
    images_list = []
    labels_list = []
    gaumaps_list = []
    gaumaps_all_list = []
    label_names_list = []
    for each_train_dir in train_dir:
        images, labels, gaumaps, gaumaps_all, label_names = load_images_labels_gaussian(each_train_dir, N, mask_filter, 
                                                                                        image_filter, gaussian_filter, 
                                                                                        foldername, variance)
        images_list.append(np.array(images))
        labels_list.append(np.array(labels))
        gaumaps_list.append(np.array(gaumaps))
        gaumaps_all_list.append(np.array(gaumaps_all))
        label_names_list.append(np.array(label_names))  

    images = np.concatenate(images_list, axis=0)
    labels = np.concatenate(labels_list, axis=0)
    gaumaps = np.concatenate(gaumaps_list, axis=0)
    gaumaps_all = np.concatenate(gaumaps_all_list, axis=0)
    ...
    if test_dir is not None:
        ...
        test_labels_list = []
        test_gaumaps_list = []
        test_gaumaps_all_list = []
        test_label_names_list = []
        for each_test_dir in test_dir:
            test_images, test_labels, test_gaumaps, test_gaumaps_all, test_label_names = load_images_labels_gaussian(each_test_dir, N, mask_filter, 
                                                                                                                    image_filter, gaussian_filter, 
                                                                                                                    foldername, variance)

            test_images_list.append(np.array(test_images))
            test_labels_list.append(np.array(test_labels))
            test_gaumaps_list.append(np.array(test_gaumaps))
            test_gaumaps_all_list.append(np.array(test_gaumaps_all))
            test_label_names_list.append(np.array(test_label_names))

        test_images = np.concatenate(test_images_list, axis=0)
        test_labels = np.concatenate(test_labels_list, axis=0)
        test_gaumaps = np.concatenate(test_gaumaps_list, axis=0)
        test_gaumaps_all = np.concatenate(test_gaumaps_all_list, axis=0)
...
def save_masks(images, masks, flows, labels, gaumaps, spots, file_names, N, variance= 7, foldername = '', png=True, tif=False, channels=[0,0],
    ...
    if isinstance(masks, list):
        for image, mask, flow, label, gaumap, spot, file_name in zip(images, masks, flows, labels, gaumaps, spots, file_names):
            save_masks(image, mask, flow, label, gaumap, spot, file_name, variance, png=png, tif=tif, suffix=suffix, dir_above=dir_above,
...
#     base = os.path.splitext(file_names)[0]
    ...
    if savedir is None: 
        savedir = str(Path(file_names).parent.parent.absolute()) + '/{}_{}_Var{}'.format(foldername, N, variance)
    ...
    with warnings.catch_warnings():
        ...
        for ext in exts:
            label_remap = fastremap.renumber(labels, in_place=True)[0]
    ...
    if png and MATPLOTLIB and not min(images.shape) > 3:
        ...
        plot.show_segmentation(fig, img, masks, flows[0], labels, gaumaps, spots)
