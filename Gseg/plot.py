...
import utils, io_img, transforms
...
def show_segmentation(fig, img, maski, flowi, labeli, gaumapi, spot, channels=[0,0], file_name=None):
    ...
    heatmap_all = cv2.applyColorMap(gaumapi.astype(np.uint8), cv2.COLORMAP_JET)
    # print(np.min(heatmap_all))
    # print(np.max(heatmap_all))
    heatmap_all[gaumapi == 0] = 0
    #image_gaumap = np.concatenate((img0, heatmap_all), axis=2)
    #image_gaumap = np.concatenate((img0, ~gaumapi[:,:,np.newaxis]), axis=2)
    ax = fig.add_subplot(1,6,1)
    #ax.imshow(image_gaumap)
    #ax.imshow(img0, alpha=0.7)
    ax.imshow(img0)
    #ax.imshow(heatmap_all, alpha=0.3)
    # X=[]
    # Y=[]
    # for x,y in spot:
    #     if maski[y,x] != 0:
    ...
    # if len(spot) !=0:
    #     ax.scatter(spot[:,0],spot[:,1],c=c, s=2)
    ...
    imgout = img0.copy()
    #imgout[outX_gt, outY_gt] = np.array([0,255,0]) # pure red
