...
def logger_setup():
    cp_dir = pathlib.Path.home().joinpath('.Gseg_pciSeq_2C_levelset')
    cp_dir.mkdir(exist_ok=True)
    log_file = cp_dir.joinpath('Gseg_levelset.log')
    try:
        log_file.unlink()
    except:
...
def get_image_files(folder, img_filter='_image'):
    ...
    # print("image_names:", image_names, len(image_names))
    if len(image_names)==0:
        raise ValueError('ERROR: no images in --dir folder')
    
    return image_names

def get_label_files(folder, N, mask_filter='_label', foldername = 'newlabels', variance = 7):
    label_names = []
...
def get_heatmap_files(folder, heatmap_filter='_gaumap_all'):
    ...
    # print("heatmap_all_names:", heatmap_all_names, len(heatmap_all_names))
...
def get_spot_files(dir):
    ...
    # print("spot_all_names:", spot_names, len(spot_names))
    return spot_names

def load_images_labels_heatmap_spot(dir, N, mask_filter='_masks', image_filter='_label', heatmap_filter='_gaumap_all', foldername = 'newlabels', variance = 7):
...
def load_train_test_data(train_dir, N, test_dir=None, image_filter='_image', mask_filter='_label', heatmap_filter='_gaumap_all', foldername = 'newlabels', variance = 7):
    ...
    # print("train_subdir_list:", train_subdir_list, len(train_subdir_list))
    ...
    if test_dir is not None:
        ...
        # print("test_subdir_list:", test_subdir_list, len(test_subdir_list))
...
def save_masks(images, masks, flows, label, spot, file_names, variance, png=True, tif=False, foldername = 'newlabels', channels=[0,0],
