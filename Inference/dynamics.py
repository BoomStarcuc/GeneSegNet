...
def find_center_condidates(centermap, offsetmap, size=[256,256]):
    ...
    joint_candi_np_array = joint_candi_np_array[joint_candi_np_array[:,2] > 0.5]
    joint_candi_np_array_withembed = joint_candi_np_array_withembed[joint_candi_np_array_withembed[:,2] > 0.5]
...
def get_mask(center_coord, offsetmap, cp_mask):
    ...
    cell_index = []
    for index in range(0, len(pre_center_coord), 100000):
        slice_index_range = range(index, min(index + 100000, len(pre_center_coord)))
        distance_map = np.ones((len(slice_index_range), center_coord.shape[0]))*np.inf
        for i, cell_center in enumerate(center_coord[:,:2]):
            distance_map[:, i] = np.sqrt(np.sum((pre_center_coord[index:min(index + 100000, len(pre_center_coord)), :] - cell_center.reshape(-1,2))**2, axis=1))
        cell_index.extend(np.argmin(distance_map, axis=1))
...
def compute_masks(offsetmap, centermap, confimap, p=None, 
    ...
    if np.any(cp_mask): #mask at this point is a cell cluster binary map, not labels     
        ...
        joint_candi_np_array, joint_candi_np_array_withembed = find_center_condidates(centermap, ofmap, 
                                                                                        size=[centermap.shape[0], centermap.shape[1]])
