...
class CellposeModel(UnetModel):
    ...
    def __init__(self, gpu=False, pretrained_model=False, 
                    net_avg=False, diam_mean=34., device=None,
    ...
    def _run_cp(self, x, compute_masks=True, normalize=True, invert=False,
        ...
        if do_3D:
            ...
            # print("iterator:", iterator) #range(0,1)
        ...
        if compute_masks:
            ...
            #print("niter:", niter) # 188.23529411764704
            if do_3D:
                ...
                for i in iterator:
                    # print("dP[:,i]:", dP[:,i].shape) #(2,256,256)
                    # print("cellprob[i]:", cellprob[i].shape) #(256,256)
    ...
    def distancemap(self, lbl, geneEs):
        lbl_reverse = lbl.copy() #(8,224,224)
        lbl_reverse[:][lbl[:] > 0] = 0
        lbl_reverse[:][lbl[:]==0] = 255

        geneEs = geneEs * lbl_reverse #(8,224,224)
        gene_dismaps = []
        for i, (ilbl, geneE) in enumerate(zip(lbl_reverse, geneEs)):
            distmap = cv2.distanceTransform(ilbl.astype(np.uint8), cv2.DIST_L2, 5) #(224,224)
            inds = np.array(np.nonzero(geneE)).astype(np.int32).T
            gene_distmap = np.zeros(distmap.shape) 
            gene_distmap[inds[:,0], inds[:,1]] = distmap[inds[:,0], inds[:,1]]
            gene_dismaps.append(gene_distmap)

        #print("gene_dismaps:", np.array(gene_dismaps).shape) (8,224,224)
        return np.array(gene_dismaps)      
    ...
    def CV (self, LSF, img, mu, nu, epison, step):
        ...
        s = self.mat_math(Ixs*Ixs+Iys*Iys, "sqrt") 
        Nx = Ixs / (s+0.000001) 
        Ny = Iys / (s+0.000001)
    ...
    def levelset(self, lbl, iters):
        '''
        lbl dtype: torch.float32 (8,224,224)
        '''
        #parameters
        mu = 1 
        nu = 0.003 * 255 * 255 
        ...
        epison = 1 
        step = 0.01
        img = lbl.unsqueeze(1) #(8,1,224,224)
        LSF= lbl.unsqueeze(1) #(8,1,224,224)

        for i in range(0,num):
            LSF = self.CV(LSF, img, mu, nu, epison, step) #iteration
        
        # print("LSF_models:", LSF.shape, LSF.requires_grad) #(8,1,224,224) False
        LSF[:][LSF[:] >= 0] = 1
        LSF[:][LSF[:] < 0] = 0
        
        lsfs = []
        for lsf in LSF:
            mask = utils.fill_holes_and_remove_small_masks(np.array(lsf.squeeze().cpu().numpy(),dtype=np.int32))
            lsfs.append(mask)

        lsfs = np.array(lsfs)
        lsfs = self._to_device(lsfs[:]>.5)
        return lsfs

    def loss_fn(self, lbl, y, geneE, iepoch):
        ...
        cellprob  = self._to_device(lbl[:,0]>.5) 
        
        # level set
        cellprob = self.levelset(cellprob, 1)
        loss2 = self.criterion2(y[:,2], cellprob)

        if iepoch > 1:
            gene_dismaps = self.distancemap(lbl[:,0], geneE)
            pred_cellprob = torch.sigmoid(y[:,2]) 
            cellmask = pred_cellprob[:] > 0.1
            pred_cellprob = pred_cellprob*cellmask 

            loss3 = 0
            count = 0
            for i,(pre_cp, gd) in enumerate(zip(pred_cellprob, gene_dismaps)):
                gd = self._to_device(gd)
                #self.plot_io(pre_cp*gd)
                logit = (pre_cp*gd).sum()
                if logit != 0:
                    loss3 += 1/logit
                    count += 1
